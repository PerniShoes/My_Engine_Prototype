...
EnemyAi::EnemyAi(int posX, int posY, DifficultySettings difficulty)
    :m_Position{ posX,posY }, m_Height{ 10 }, m_Width{ 10 }, m_Velocity{ 300.0f }, m_VelocityY{ 0 }, m_VelocityX{ 0 }
{
...
void EnemyAi::move(int borderGirth, DifficultySettings difficulty, SDL_Point ballPos, float ballVelocity)
{
    using enum DifficultySettings;

    setVelocity(300.0f+(160.f*(float)difficulty));
    ...
    if (difficulty == unbeatable || m_VelocityY > ballVelocity-250.0f)
    {
